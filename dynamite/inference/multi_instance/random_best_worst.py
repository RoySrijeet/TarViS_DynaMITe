...
def evaluate(
    ...
    print(f'[EVALUATOR INFO] TarViS CONFIGURATION:\n {tarvis_config}')
    ...
    with ExitStack() as stack:                                           
        ...
        for seq in list(dataloader_dict.keys()):
            ...
            tarvis_config['mask_directory'] = os.path.join(expt_path,'Annotations')
            ...
            while lowest_frame_index!=-1:
                ...
                clicker.save_visualization(vis_path_round, ious=ious, num_interactions=num_interactions_for_sequence[lowest_frame_index], round_num=round_num, save_masks=True, expt_path=tarvis_config['output_dir'], seq_name=seq)
                ...
                # TarViS
                tarvis_config['seq_names'] = [seq]                
                out_masks = eval_tarvis(tarvis_config)

                # out_masks - a list of dicts, one dict for each frame
                # each dict has a unique key for each instance in the frame
                # each key contains the binary mask for the instance as its value
                out_masks_ = []
                H,W = out_masks[0][list(out_masks[0].keys())[0]].shape

                for idx in range(len(out_masks)): 
                    dummy_ = np.zeros((H,W))
                    for k in out_masks[idx].keys():
                        dummy_ += out_masks[idx][k] * k
